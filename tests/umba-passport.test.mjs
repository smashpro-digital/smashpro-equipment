...
test("approved projection contains only public facts and does not advance validation", () => {
  ...
  assert.equal(publicRecord.runtime.label, "Lifetime machine runtime");
  assert.equal(publicRecord.runtime.verified_runtime_minutes, 563);
  assert.equal(publicRecord.runtime.verified_runtime_hours, 9.38);
  assert.equal(publicRecord.runtime.known_completed_runs, 6);
  assert.equal(publicRecord.runtime.pending_runtime_runs, 4);
  ...
  assert.equal(item.publicRuntime.verifiedRuntimeMinutes, 563);
  ...
  assert.ok(item.timeline.some(row => row.id === "runtime-history-backfill" && row.kind === "hours"));
