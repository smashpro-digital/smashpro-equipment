# SP-MZIGO-26E Procurement Record

> **Canonical status:** This document is the procurement source of truth for Fleet Asset **SP-MZIGO-26E** until superseded by official factory documentation. Values not yet supplied by the vendor are marked pending and must not be estimated.

## 1. Procurement Summary

| Field | Details |
| --- | --- |
| Vendor | Shandong Kylin Heavy Industry Machinery Co., Ltd. |
| Address | Not documented |
| Website | Not documented |
| Email | Not documented |
| Telephone | Not documented |

## 2. Proforma Invoice

| Field | Details |
| --- | --- |
| Document | Not documented |
| Invoice number | Not documented |
| Date | Not documented |
| Shipping terms | Not documented |

### Purchaser

Not documented

## 3. Machine Specifications

| Field | Documented specification |
| --- | --- |
| Platform model | K600 — documented in SmashPro passport records; pending manufacturer specification sheet |
| Payload capacity | 500 kg (1,102 lb) |
| Drive | 4WD Electric |
| Operation | Remote controlled |
| Dump bed | Hydraulic electric dump |

The **K600** platform designation is recorded from SmashPro's own equipment passport documentation. It is not yet corroborated by a signed proforma invoice or manufacturer specification sheet and must remain marked pending until one is received.

## 4. Observed Production Changes

| Field | Record |
| --- | --- |
| Black wheel finish | Installed; photographed September 14–15, 2026 |
| Black battery boxes | Installed; photographed September 14, 2026 |
| Tie-down anchors | Installed; dedicated factory detail received September 14, 2026 |
| Custom branding and decals | Installed and retained in September 15 completed profile |
| Equipment Passport QR panel | Installed and retained in September 15 completed profile |
| Quality inspection | Passed, per supplied production status |
| SmashPro build approval | Approved, per supplied production status |

## 5. Included Equipment

- Hydraulic dump bed
- Remote controller
- Toolbox
- English user manual
- Two battery chargers
- Spare drive shaft
- Spare tire
- Remote control batteries

## 6. Included Accessories

| Accessory | Invoice value | Amount charged |
| --- | ---: | ---: |
| Not documented | Not documented | Not documented |

## 7. Customization

| Field | Approved customization |
| --- | --- |
| Factory color | SmashPro Custom Green |
| Branding | Custom SmashPro logo decals |
| Asset name | SP-MZIGO-26E |

## 8. Pricing

Pricing has not been documented in the repository for SP-MZIGO-26E. Machine price, accessory value, and total DDP amount remain pending vendor documentation.

## 9. Payment Terms

Deposit paid, confirmed by the SmashPro Equipment Passport build status. Final payment is reported pending in the September 15 production status. Amounts, exact due date and remaining terms are not documented publicly.

## 10. Delivery

| Field | Terms |
| --- | --- |
| Packaging | Not documented |
| Destination | South Carolina, USA |
| Delivery estimate | Not documented |

## 11. Warranty

Factory warranty is **1 year or 1,500 working hours**, whichever occurs first.

## 12. Bank Information

Not documented. Banking instructions must be independently verified with the vendor through a trusted channel before any transfer. A change to beneficiary or account details requires new official documentation and independent verification.

## 13. Procurement Timeline

| Milestone | Record status | Date / reference |
| --- | --- | --- |
| Vendor selection | Recorded | Not documented |
| Deposit paid | Recorded | Not documented |
| Custom branding approved | Recorded | Not documented |
| SmashPro Custom Green finish approved | Recorded | Not documented |
| Production started | Recorded | Not documented |
| Equipment Passport published | Recorded | 2026-08-20 |
| Product website published | Recorded | Not documented |
| Powder coating / green finish | Recorded | Factory media through 2026-09-15 |
| Decal and QR installation | Recorded | Factory media through 2026-09-15 |
| Black wheel and battery-box revisions | Recorded | 2026-09-14 factory evidence |
| Tie-down anchors | Recorded | 2026-09-14 factory evidence |
| Quality inspection | Passed | Supplied 2026-09-15 production status |
| SmashPro build approval | Approved | Supplied 2026-09-15 production status |
| Final payment | Pending | Amount, due date and terms not published |
| Export crating | Pending | Not documented |
| Port delivery | Pending | Not documented |
| Vessel booking / ocean departure | Pending | Not documented |
| Ocean freight | Pending | Not documented |
| U.S. delivery | Pending | Not documented |

## 14. Future Documentation Register

| Document or identifier | Status | Value / reference |
| --- | --- | --- |
| Proforma invoice | Pending | To be added |
| Manufacturer specification sheet (K600 confirmation) | Pending | To be added |
| Shipping documents | Pending | To be added |
| Bill of lading | Pending | To be added |
| Container number | Pending | To be added |
| Serial number | Pending | To be added |
| Motor serial numbers | Pending | To be added |
| Battery specification sheet | Pending | To be added |
| Factory QC report | Pending | To be added |
| Operator manual (on file copy) | Pending | To be added |
| Maintenance schedule | Pending | To be added |
| Arrival inspection | Pending | To be added |

When a record is received, preserve the original file, record its date and source, and link or identify it here. Do not replace prior evidence without retaining revision history.

## 15. Canonical Status and Change Control

This document is the procurement source of truth for **SP-MZIGO-26E** until superseded by official factory documentation.

Any specification recorded here that is not yet confirmed by a signed proforma invoice or manufacturer specification sheet — most notably the K600 platform designation — must remain marked pending until official documentation is received. When official documentation is received:

1. Preserve the prior documented value and the superseding document reference.
2. Record the effective date and the person who verified the change.
3. Update the applicable specification only after verification.
4. Keep unconfirmed observations clearly separated from documented specifications.
