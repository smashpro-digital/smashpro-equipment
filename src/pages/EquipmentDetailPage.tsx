...
export function EquipmentDetailPage({ slug }: { slug: string }) {
  ...
  return (
    <PageFrame><SeoSchema item={item} />
      ...
      <section className={`detail-hero${isArdhiFlagship ? " detail-hero--ardhi" : ""}${isMzigoElectric ? " detail-hero--electric" : ""}`}><img src={item.heroImage} alt={`${item.fleetId} ${item.category}`} width="1536" height="1024" /><div className="detail-hero__shade" /><div className="shell detail-hero__content">
