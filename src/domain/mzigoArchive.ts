import type { EvidenceCategory, GalleryGroup, GalleryImage } from "../types/equipment";
...
export const mzigoArchiveChapters: { id: string; label: string; groups: GalleryGroup[]; description: string; empty: string }[] = [
  ...
  { id: "finished-machine", label: "Finished Machine", groups: ["branding", "completed-machine"], description: "SmashPro green, installed graphics and QR panel, black wheels and battery boxes, tie-down anchors, remote controller, and the build-approved profile.", empty: "Finished-machine evidence will appear here as it is documented." },
  { id: "export-journey", label: "Export Journey", groups: ["export", "shipping"], description: "The next stage after final payment and build approval.", empty: "Export is still ahead. No crate, port, shipment or vessel record has been published for MZIGO." },
...
export function mzigoArchiveSelection(gallery: GalleryImage[], chapterId: string, query = "", category?: EvidenceCategory) {
  const term = query.trim().toLowerCase();
  return gallery.filter(media => mzigoChapterForMedia(media)?.id === chapterId && (!category || media.evidenceCategory === category) && `${media.alt} ${media.caption ?? ""} ${media.group ?? ""} ${media.evidenceCategory ?? ""} ${media.capturedAt ?? ""} ${media.kind === "video" ? "video" : "photo image"}`.toLowerCase().includes(term))
