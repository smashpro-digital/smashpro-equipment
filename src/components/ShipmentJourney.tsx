...
export function ShipmentJourney({ result, refresh }: { result: ShipmentResult; refresh: () => void }) {
  ...
  return <section id="journey" className="shipment-journey" aria-labelledby="shipment-journey-title">
    <div className="shell">
      ...
      {!data?.currentPosition && <div className="shipment-imagery-empty"><span aria-hidden="true">◉</span><div><strong>No approved vessel-position observation available</strong><p>AIS observations require a timestamped provider record and approval for public display. The reference map is cartography and does not simulate vessel movement.</p></div></div>}
