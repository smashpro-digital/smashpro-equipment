...
export function CommissioningPassport({ item }: { item: Equipment }) {
  ...
  const [liveRuntime, setLiveRuntime] = useState<{
    ...
    verified_runtime_runs: number;
    pending_runtime_runs: number;
    failed_runs: number;
  ...
  const verifiedMinutes = liveRuntime?.verified_runtime_minutes ?? runtime?.verifiedRuntimeMinutes ?? runtime?.verifiedBaselineMinutes ?? 0;
  const verifiedHours = liveRuntime?.verified_runtime_hours ?? runtime?.verifiedRuntimeHours ?? runtime?.verifiedBaselineHours ?? 0;
  ...
  return <div className="commissioning-passport">
    ...
    {runtime && <section className="section shell" id="runtime"><p className="eyebrow">Lifetime utilization</p><h2>{runtimeClock} verified machine runtime.</h2><p>Commissioning, internal, prototype, production and reprint activity all contribute to the equipment odometer. A completed run counts immediately even when its duration is still awaiting backfill. Printer completion never passes QC or closes manufacturing work.</p><dl className="spec-grid passport-spec-grid"><div><dt>Verified runtime</dt><dd>{verifiedHours.toFixed(2)} hours</dd></div><div><dt>Verified minutes</dt><dd>{verifiedMinutes} min</dd></div><div><dt>Completed runs</dt><dd>{liveRuntime ? liveRuntime.completed_runs : "6 known"}</dd></div><div><dt>Runtime pending</dt><dd>{liveRuntime ? liveRuntime.pending_runtime_runs : "4 runs"}</dd></div><div><dt>Tracking state</dt><dd>{runtime.trackingStatus.replaceAll("_", " ")}</dd></div></dl><div className="commissioning-callout"><h3>Automatic tracking path</h3><p>{liveRuntime ? "Digital HQ runtime rollup is online." : "Using the verified static lifetime total until the live rollup responds."} The local FlashForge adapter still requires live edge commissioning before unattended printer-to-passport tracking is complete.</p>{(liveRuntime?.runtime_completeness ?? runtime.additionalRuntimeStatus) === "partial_backfill" || runtime.additionalRuntimeStatus === "pending_backfill" ? <p>Some completed jobs are awaiting runtime backfill. They are counted as completed runs but contribute no invented minutes to the verified total.</p> : null}</div>{liveRuntime?.recent_runs?.length ? <div className="commissioning-run-log"><h3>Recent machine-run log</h3><ol>{liveRuntime.recent_runs.map(run => <li key={run.machine_run_id}><div><code>{run.file_name}</code><span>{run.completed_at ? new Date(run.completed_at).toLocaleString() : "Completion time unavailable"}</span></div><strong>{run.runtime_minutes === null ? "Runtime pending" : `${run.runtime_minutes} min`}</strong></li>)}</ol></div> : null}</section>}
