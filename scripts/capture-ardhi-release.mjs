...
for (const width of [320, 375, 390, 430, 768, 1024, 1440]) {
  ...
  const metrics = await client.send("Runtime.evaluate", { expression: `(() => { const links=[...document.querySelectorAll('.equipment-projects a[href]')]; const ids=[...document.querySelectorAll('[id]')].map(n=>n.id); const headings=[...document.querySelectorAll('h1,h2,h3,h4,h5,h6')].map(n=>Number(n.tagName.slice(1))); const first=links.find(a=>a.getAttribute('href')?.startsWith('/book/')); first?.focus(); return {innerWidth,scrollWidth:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth),cards:document.querySelectorAll('.project-grid article').length,brokenImages:[...document.images].filter(i=>i.complete&&i.naturalWidth===0).length,missingAlt:[...document.images].filter(i=>!i.hasAttribute('alt')).length,duplicateIds:ids.filter((id,i)=>ids.indexOf(id)!==i).length,headingSkips:headings.filter((level,i)=>i&&level>headings[i-1]+1).length,noAppSchemeLinks:links.every(a=>!a.getAttribute('href')?.startsWith('smashpro-home:')),ctaValid:Boolean(first?.getAttribute('href')?.includes('service=')&&first.getAttribute('href')?.includes('equipment_source=SP-ARDHI-26')&&first.getAttribute('href')?.includes('equipment_required=SP-ARDHI-26')),utmPreserved:Boolean(first?.getAttribute('href')?.includes('utm_source=release')&&first.getAttribute('href')?.includes('utm_campaign=ardhi26')&&first.getAttribute('href')?.includes('utm_medium=web')&&first.getAttribute('href')?.includes('utm_content=passport-release')),keyboardFocus:document.activeElement===first}; })()`, returnByValue: true });
...
if (results.some((r) => r.scrollWidth > r.innerWidth || r.cards !== 9 || r.brokenImages || r.missingAlt || r.duplicateIds || r.headingSkips || !r.noAppSchemeLinks || !r.ctaValid || !r.utmPreserved || !r.keyboardFocus || r.errors)) process.exitCode = 1;
